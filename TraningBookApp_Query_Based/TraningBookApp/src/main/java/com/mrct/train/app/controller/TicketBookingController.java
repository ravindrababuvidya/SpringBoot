...
public class TicketBookingController {
	...
	public String deletTicketAll() {
		 ticketBookingService.deletTicketAll();
		 return "Sucessfully Deleted All Records ";
	}
	@GetMapping(value = "passengerName/{passengerName}")
	public Ticket getTicketByPassengerName(@PathVariable(name = "passengerName") String passengerName) {
		return ticketBookingService.getTicketByPassengerName(passengerName);
	}
	@GetMapping(value = "email/{email}")
	public Ticket getTicketByEmail(@PathVariable(name = "email") String email) {
		System.out.println("URLEncoder.encode(email)"+URLEncoder.encode(email));
